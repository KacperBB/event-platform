...
export default async function EventPage({ params }: { params: Promise<{ eventId: string }> }) {
    ...
    const spotsLeft = event.maxCapacity ? event.maxCapacity - totalOccupancy : Infinity;
    const canBook = (!event.maxCapacity || spotsLeft > 0) && (!event.bookingDeadline || new Date() < event.bookingDeadline);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <section>
              ...
              <p className="text-xl text-muted-foreground mt-2">{event.address}</p>
            </section>

            <article className="prose prose-slate max-w-none dark:prose-invert">
              <h3 className="text-2xl font-semibold mb-4 text-slate-800">O wydarzeniu</h3>
              <div dangerouslySetInnerHTML={{ __html: event.description || "" }} />
            </article>

            <section className="h-[400px] w-full rounded-2xl overflow-hidden border shadow-inner">
              <EventMap lat={event.lat} lng={event.lng} title={event.title} address={event.address} />
            </section>
          </div>
          ...
          <aside className="space-y-6">
            <div className="sticky top-6 p-6 border rounded-3xl bg-white shadow-sm space-y-4">
              ...
              <div className="flex flex-col gap-1 text-center py-2">
                 <span className="text-2xl font-bold">{event.maxCapacity ? `${spotsLeft}` : "∞"}</span>
                 <span className="text-xs text-muted-foreground uppercase tracking-widest font-bold">Dostępnych miejsc</span>
              </div>
